...
def compute_torques(
    ...
    for angles in angle_combinations:
        ...
        if np.any(np.abs(numerical_torques) > max_torque_per_joint):
            ...
            top_configuration = (angles, numerical_torques)
    ...
    # Plot the top configuration
    angles, torques = top_configuration
    fig = plt.figure(figsize=(8, 8))
    ax = fig.add_subplot(111, projection="3d")

    # Evaluate transformation matrices
    T1_eval = np.array(T1.subs(values).subs({theta_1: angles[0]})).astype(np.float64)
    T2_eval = np.array(
