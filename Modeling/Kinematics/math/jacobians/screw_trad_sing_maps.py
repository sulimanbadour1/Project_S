...
def compute_determinant(J):
    return np.linalg.det(J)


def plot_determinants(det_traditional, det_screw, theta_samples):
    plt.figure(figsize=(12, 6))
    plt.plot(theta_samples, det_traditional, label="Traditional Method")
    plt.plot(theta_samples, det_screw, label="Screw Axis Method", linestyle="--")
...
def plot_condition_numbers(cond_traditional, cond_screw, theta_samples):
    plt.figure(figsize=(12, 6))
    plt.plot(theta_samples, cond_traditional, label="Traditional Method")
    plt.plot(theta_samples, cond_screw, label="Screw Axis Method", linestyle="--")
    plt.xlabel("Sample Index")
    plt.ylabel("Condition Number")
    plt.title("Condition Number of Jacobian")
    plt.legend()
    plt.grid(True)
    plt.show()


########## Visualize the Jacobian Matrices

import seaborn as sns


def plot_jacobian_heatmap(J, title):
    plt.figure(figsize=(10, 8))
    sns.heatmap(J, annot=True, cmap="coolwarm")
    plt.title(title)
    plt.xlabel("Joint Variables")
    plt.ylabel("End Effector Position and Orientation")
    plt.show()
